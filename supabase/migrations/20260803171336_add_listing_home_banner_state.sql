alter table public.listings
  add column if not exists home_banner_until timestamp with time zone,
  add column if not exists home_banner_slot integer;

create index if not exists idx_listings_home_banner_until_slot
  on public.listings (home_banner_until desc, home_banner_slot asc)
  where home_banner_until is not null;
...
select
  ...
  case when l.home_banner_until is not null and l.home_banner_until > now() then true else false end as is_home_banner,
  l.home_banner_until,
  l.home_banner_slot
