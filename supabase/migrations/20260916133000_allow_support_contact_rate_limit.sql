-- Allow authenticated support form submissions through the existing database-backed rate limiter.
-- Keep all existing action contracts unchanged and add only the support_contact rule.
...
begin
  ...
  if not (
    ...
    or (p_action = 'support_contact' and p_window_seconds = 3600 and p_max_hits = 5)
  ) then
